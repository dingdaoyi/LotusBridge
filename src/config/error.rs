...
impl Display for EdgeError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            EdgeError::AuthError(auth) => match auth {
        ...
        let (status, error_message) = match self {
            ...
            EdgeError::SqlxError(err) => {
                ...
                (
                    ...
                    R::fail(format!("数据库异常:{}", err.to_string())),
                )
            }
            ...
            EdgeError::ValidationErrors(validation_errors) => (
                ...
                R::bad_request(validation_errors.to_string()),
            ),
            EdgeError::AuthError(auth) => match auth {
                AuthError::WrongCredentials => (
                    StatusCode::UNAUTHORIZED,
                    R::fail_with_code(401, "Wrong credentials".to_string()),
                ),
                AuthError::MissingCredentials => (
                    StatusCode::FORBIDDEN,
                    R::fail_with_code(403, "Missing credentials".to_string()),
                ),
                AuthError::TokenCreation => (
                    StatusCode::INTERNAL_SERVER_ERROR,
                    R::fail_with_code(500, "Token creation error".to_string()),
                ),
                AuthError::InvalidToken => (
                    StatusCode::UNAUTHORIZED,
                    R::fail_with_code(401, "Invalid token".to_string()),
                ),
            },
        };
