...
pub async fn login(Json(payload): Json<LoginPayload>) -> Result<Json<R<String>>, EdgeError> {
    ...
    let token = generate_jwt_token(&payload.username, &auth_config.jwt_secret, auth_config.expire_minutes)?;
    // 将 JWT 令牌发送到响应中
...
fn generate_jwt_token(username: &str, jwt_secret: &str, expire_minutes: usize) -> Result<String, AuthError> {
